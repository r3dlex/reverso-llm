...
Reverso is a subscription-backed local LLM gateway. It runs on `127.0.0.1:64946` and serves the OpenAI Responses API natively through a first-party gateway app (`reverso.protocols.responses_app.ResponsesGatewayApp`) with one adapter per provider: claude (subscription OAuth via the claude CLI), copilot (direct upstream forward), auggie (auggie CLI), and deepseek (first-party HTTP). See ADR 0002 and ADR 0003 under `docs/architecture/adr/`.

Reverso also serves an inbound Anthropic Messages API surface (`/v1/messages`) on the same port for Claude Code and the Claude Agent SDK via `ANTHROPIC_BASE_URL`; it is inbound only (no `api.anthropic.com` upstream), model-routes to the claude, copilot, deepseek, and auggie backends through a single first-party authority (`surface_registry`), and translates onto the frozen Responses contract without changing the `ProviderAdapter` Protocol. The claude backend is served first-party via the local `claude` CLI under subscription OAuth (ADR 0009, superseding ADR 0006 D2; the spawned CLI has `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`/`ANTHROPIC_API_KEY` scrubbed so it never loops back into Reverso). See ADR 0009 and ADR 0006 under `docs/architecture/adr/`. The legacy LiteLLM app remains composed behind the first-party app as fallthrough only for paths the gateway does not own. Wrapped CLI session management (Claude Code CLI, Codex CLI) is handled by the session daemon. MiniMax is direct Codex-only and is not routed through Reverso.
...
This repo follows the v3 AI-SDLC layout (`topology_type: standalone`, depth 0). It is the single source of truth for the agent operating contract; `CLAUDE.md` and `GEMINI.md` are thin pointers to this file (ADR-0014). See `.ai/matrix.json`, `.memory/human-override/`, and `docs/architecture/adr/`. Modules at `r3dlex/skills/init-ai-repo/modules/`.
...
The six context types this harness assembles, and where each lives (ADR-0015):
