...
Reverso also serves an inbound Anthropic Messages API surface (`/v1/messages`) on the same port for Claude Code and the Claude Agent SDK via `ANTHROPIC_BASE_URL`; it is inbound only (no `api.anthropic.com` upstream), model-routes to the claude, copilot, deepseek, and auggie backends through a single first-party authority (`surface_registry`) — the claude backend is served first-party via the local `claude` CLI under subscription OAuth (ADR 0008, superseding ADR 0006 D2; the spawned CLI has `ANTHROPIC_BASE_URL`/`ANTHROPIC_AUTH_TOKEN`/`ANTHROPIC_API_KEY` scrubbed so it never loops back into Reverso) — and translates onto the frozen Responses contract without changing the `ProviderAdapter` Protocol. See ADR 0008 and ADR 0006 under `docs/architecture/adr/`. The legacy LiteLLM app remains composed behind the first-party app as fallthrough only for paths the gateway does not own. Wrapped CLI session management (Claude Code CLI, Codex CLI) is handled by the session daemon. MiniMax is direct Codex-only and is not routed through Reverso.
