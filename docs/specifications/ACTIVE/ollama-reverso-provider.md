...
- The authority publishes bare ids (`gpt-oss:120b`). The user-owned local Ollama service routes the same model only under the documented `:cloud` (colon) alias (`gpt-oss:120b:cloud`); the bare id and the earlier documented `<id>-cloud` (hyphen) alias are both rejected locally with `model '<id>' not found`. Reverso therefore publishes `<authority-id>:cloud` as the routing raw id and never double-suffixes an authority id that already carries the colon alias. This is a documented alias applied to an authority-published id, not inference over an unlabelled local row. Tested minimum Ollama version: 0.32.14 (revision 2026-08-22; see ADR-0019).
