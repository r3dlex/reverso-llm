...
# hijacked credential. We strip them from the CHILD env only so the CLI always
# reaches api.anthropic.com under its own subscription OAuth; the parent
# os.environ is never mutated (the falsifiable OAuth gate asserts that).
_LOOP_PREVENTION_ENV = (
    "ANTHROPIC_BASE_URL",
...
def _child_env_for_cli(token: str) -> dict[str, str]:
    ...
    CLI back into Reverso (infinite loop) and no caller-provided
    ANTHROPIC_AUTH_TOKEN/ANTHROPIC_API_KEY can hijack the request. Only after the
    scrub is the live subscription token injected. The parent os.environ is left
    untouched: only the returned copy is mutated.
    """
    child_env = dict(os.environ)
    for key in _LOOP_PREVENTION_ENV:
