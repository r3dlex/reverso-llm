...
RESPONSES_PROVIDERS = tuple(sorted(APP_PROVIDER_PREFIXES))
...
def _client(adapters: Mapping[str, ProviderAdapter]) -> httpx.AsyncClient:
    app = build_app(dict(adapters))
...
@pytest.mark.asyncio
@pytest.mark.parametrize("provider", RESPONSES_PROVIDERS)
async def test_responses_nonstreaming_dispatches_compressed_request_for_each_provider(
    provider: str,
    ...
    async with _client(adapters) as client:
        ...
        input_items = await client.get(f"/{provider}/v1/responses/resp_spy/input_items")
    ...
    assert sorted(untouched) == sorted(set(RESPONSES_PROVIDERS) - {provider})


@pytest.mark.asyncio
@pytest.mark.parametrize("provider", RESPONSES_PROVIDERS)
async def test_responses_streaming_dispatches_compressed_request_for_each_provider(
    provider: str,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    adapter = SpyAdapter()
    ...
    async with _client({provider: adapter}) as client:
        ...
        input_items = await client.get(
            f"/{provider}/v1/responses/resp_stream/input_items"
        )
...
async def test_feature_gate_runs_before_headroom(
    ...
    async with _client({"claude": adapter}) as client:
        resp = await client.post(
            "/claude/v1/responses",
            json={
                "model": "m",
