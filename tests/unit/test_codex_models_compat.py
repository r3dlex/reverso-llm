...
def test_codex_models_compat_adds_models_field() -> None:
    ...
    assert sent[0]["headers"][-1] == (b"content-length", str(len(sent[1]["body"])).encode("ascii"))
...
def test_codex_models_compat_leaves_non_codex_models_response_unchanged() -> None:
    ...
    asyncio.run(CodexModelsCompatMiddleware(app)({"type": "http", "path": "/v1/models", "query_string": b""}, receive, send))
